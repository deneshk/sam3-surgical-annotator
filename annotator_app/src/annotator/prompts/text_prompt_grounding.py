"""Helpers for converting text-prompt grounding outputs into UI proposals."""
...
class TextPromptProposal:
    """One candidate object proposal returned from a text-grounding request."""
...
def build_text_prompt_proposals(
    *,
    ...
    """Filter and convert raw SAM text-prompt outputs into UI-friendly proposals."""
...
def next_prompt_object_name(prompt: str, existing_names: Iterable[str]) -> str:
    """Choose a stable object name for accepted text-prompt proposals."""
...
def _box_xyxy_from_norm(
    ...
    """Convert a normalized xywh box to clamped pixel-space xyxy coordinates."""
