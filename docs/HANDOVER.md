...
- `annotator_app/src/annotator/propagation/`: pure helpers for prompt payloads, frame-output merges, smart propagation, and runtime state records.
- `annotator_app/src/annotator/persistence/`: session schema, JSON mapping, mask I/O, and repository boundary.
- `annotator_app/src/annotator/prompts/text_prompt_grounding.py`: text-prompt proposal helpers.
...
- Generate temporary current-frame proposals from a text prompt, then accept selected proposals into real tracked objects.
- Flag review frames.
...
- Text-prompt proposal generation uses a fresh 1-frame session.
- Tracker propagation uses chunk-local sessions by default.
- Experimental one-session propagation reuses one full-video session across chunks in a single run.
- Experimental reconditioning settings are hot-applied to the live SAM model.
...
Experimental smart propagation:

- Only applies in tracker mode.
- Requires `Use Target Frame`.
- Is disabled while one-session chunked propagation is enabled.
- When a previously seen object disappears, the app can rewind a few frames and launch a larger recovery chunk.
...
- smart propagation decision logic
