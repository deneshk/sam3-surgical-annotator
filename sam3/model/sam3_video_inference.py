...
class Sam3VideoInferenceWithInstanceInteractivity(Sam3VideoInference):
    ...
    def add_tracker_new_mask(
        ...
        """Add a new mask prompt to Tracker for one object on one frame."""
        ...
        if not isinstance(mask_inputs, torch.Tensor):
            mask_inputs = torch.tensor(mask_inputs, dtype=torch.float32)
        if mask_inputs.dim() == 3 and mask_inputs.shape[0] == 1:
            mask_inputs = mask_inputs.squeeze(0)
        if mask_inputs.dim() != 2:
            raise ValueError("mask_inputs must be a 2D mask")
