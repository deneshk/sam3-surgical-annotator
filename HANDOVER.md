...
- `Prop` auto-turns off and disables when that object has no point or box prompt on the current frame.
- Prompt modes:
...
| Click **Propagate** | For each chunk, creates a fresh N-frame PIL session from seed frame, injects current seed-frame points/box plus optional stored seed mask guidance, runs forward propagation, remaps session indices to absolute frame indices, and preserves the current seed frame instead of overwriting it |
...
4. `tools/README_surgical_annotator.md`
5. `tools/SURGICAL_ANNOTATOR_PLAN.md`
6. `sam3/model/sam3_video_predictor.py`
7. `sam3/model/sam3_video_inference.py`
8. `sam3/model/io_utils.py`
...
| 2026-03-23 | Added inline box locking, `Lock All Boxes`, selected-row white text, and automatic disabling of `Prop` when no current-frame prompt exists |
